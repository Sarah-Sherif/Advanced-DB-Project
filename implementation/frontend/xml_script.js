...
function fetchDataAndCreateTable() {
  fetch("http://127.0.0.1:5500/implementation/frontend/test.xml", {
    mode: "no-cors",
  })
      ...
      if (books.snapshotLength > 0) {
        ...
        headers.forEach((header) => {
          if (header.nodeType === Node.ELEMENT_NODE) {
            ...
            th.appendChild(document.createTextNode(header.nodeName));
      ...
      for (let i = 0; i < books.snapshotLength; i++) {
        ...
        book.childNodes.forEach((bookProperty) => {
          if (bookProperty.nodeType === Node.ELEMENT_NODE) {
            ...
            td.appendChild(document.createTextNode(bookProperty.textContent));
            row.appendChild(td);
          }
        });
